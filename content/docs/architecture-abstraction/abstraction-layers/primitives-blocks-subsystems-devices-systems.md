...
Five levels of abstraction appear repeatedly in electronics, whether or not anyone names them explicitly. They aren't a formal taxonomy — real circuits don't carry labels announcing which level they belong to — but they capture genuine differences in the kind of reasoning that works at each scale. Learning to recognize which level the current task operates at, and what kind of questions belong there, is one of the most useful structural skills in electronics.

## Primitives

A primitive is a single component: the thing that fits between tweezers, appears on a BOM line, and is characterized from a datasheet. A resistor, capacitor, inductor, diode, transistor, or crystal. At this level, reasoning centers on voltage, current, impedance, power dissipation, temperature coefficients, and parasitic properties. The datasheet is the contract: it states what the component promises under specified conditions.
...
A device is a self-contained product or module with defined external interfaces. A bench power supply. An oscilloscope. A development board. A sensor module with a digital output. A USB-to-serial adapter. At this level, reasoning centers on capabilities, interfaces, operating modes, and integration constraints.
...
A system is multiple devices working together to accomplish a purpose. A test setup: DUT, power supply, signal generator, oscilloscope, and a PC running control software. A data acquisition system: sensors, signal conditioning modules, an ADC board, and a processing unit. A control loop: sensor, controller, actuator, and the physical process being controlled. At this level, reasoning centers on coordination, sequencing, data flow, timing relationships between devices, and emergent behavior.
...
These five levels aren't a rigid classification system. The boundaries between them are sometimes blurry: is an op-amp IC a primitive or a block? (It depends on whether the reasoning concerns its internal compensation or its closed-loop gain.) Is a development board a device or a subsystem? (It depends on whether it serves as a standalone test platform or is integrated into a larger system.)

The value isn't in categorizing things perfectly — it's in recognizing that different questions belong at different levels, and that trying to answer a subsystem question with primitive-level reasoning (or vice versa) wastes time and generates confusion. Staring at oscilloscope traces of individual gate transitions while the real problem is that the power supply droops under load is a sign of working at the wrong level.
...
- Name the level being worked at, even if only internally. "Checking the block-level transfer function" or "looking at system-level timing" keeps the focus on the right questions.
